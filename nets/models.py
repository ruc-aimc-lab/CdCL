...
def build_model(model_name, training_params):
    ...
    model = getattr(Models, model_name)(custom_pretrained=custom_pretrained, training_params=training_params)
    return model

 
class Models(object):
    @staticmethod
    def cdcl_effi_b3_p(custom_pretrained, training_params):
