...
uwf_collection = 'train_uwf_md'
cfp_collection = 'train_cfp_md'
sample_num = 15
...
out_root = './heat_map__{}_{}'.format(uwf_collection, cfp_collection) 
...
model = VITMixupWholeTraining(backbone=backbone, n_class=8, channels=1536, 
...
idx_cfp = load_gt(cfp_collection, mappings)

for i in idx_uwf:
    for j in idx_cfp:
        print(i, j)
        uwf_lst = random.sample(idx_uwf[i], sample_num)
        cfp_lst = random.sample(idx_cfp[j], sample_num)
        uwfs, uwf_paths = load_img(uwf_lst, h=512, w=w)
        cfps, cfp_paths = load_img(cfp_lst, 512, 512)
        for k in range(sample_num):
            for l in range(sample_num):
                uwf = uwfs[k:k+1]
                cfp = cfps[l:l+1]
                new_cfp = pad_cfp(cfp=cfp, clarus_whole=uwf)
                mix_clarus = 0.3 * new_cfp + 0.7 * uwf


                score, score_vit, score_whole, weight_fusion, attns = model.draw_heat(mix_clarus)
                ...
                print(score_vit.shape, score_vit)
                print(score_whole.shape, score_whole)
                print(weight_fusion.shape, weight_fusion)'''

                os.makedirs(os.path.join(out_root, '{}_{}'.format(i, j), '{}_{}'.format(k, l), 'heat'), exist_ok=True)

                shutil.copy(cfp_paths[l], os.path.join(out_root, '{}_{}'.format(i, j), '{}_{}'.format(k, l), cfp_paths[l].split(os.sep)[-1]))
                shutil.copy(uwf_paths[k], os.path.join(out_root, '{}_{}'.format(i, j), '{}_{}'.format(k, l), uwf_paths[k].split(os.sep)[-1]))
                np.save(os.path.join(out_root, '{}_{}'.format(i, j), '{}_{}'.format(k, l), 'heat', 'att1.npy'), attns1)
                np.save(os.path.join(out_root, '{}_{}'.format(i, j), '{}_{}'.format(k, l), 'heat', 'att2.npy'), attns2)

                with open(os.path.join(out_root, '{}_{}'.format(i, j), '{}_{}'.format(k, l), 'scores_weights.txt'), 'w') as fout:
